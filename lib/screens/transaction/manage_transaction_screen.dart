...
class _TransactionScreenState extends State<TransactionScreen> {
  ...
  Future<void> _delete() async {
    ...
    try {
      ...
      await TransactionService().deleteTransaction(orgId, widget.transaction!.id, by: auth.firebaseUser?.uid);
      Navigator.of(context).pop(true);
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(isEdit ? 'Transaction' : 'Add Transaction')),
      body: SafeArea(
        child: Padding(
          ...
          child: Column(
            children: [
              ToggleButtons(
                ...
                children: const [Padding(padding: EdgeInsets.symmetric(horizontal: 16), child: Text('Expense')), Padding(padding: EdgeInsets.symmetric(horizontal: 16), child: Text('Fund'))],
              ),
                  ...
                  child: Form(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (orgId == null)
                          ...
                          FutureBuilder<QuerySnapshot>(
                            ...
                            builder: (context, snap) {
                              if (snap.connectionState == ConnectionState.waiting) return const SizedBox(height:24, width:24, child: Center(child: CircularProgressIndicator(strokeWidth:2)));
                              ...
                              for (final d in docs) {
                                ...
                                if (_tabIndex == 0) {
                                  if (cm.type == CategoryType.expense) catsMap[cm.id] = cm;
                                } else {
                              ...
                              if (_categories.isEmpty) {
                                ...
                                _categories = (_tabIndex == 0
                                        ? defaultExpenseCategories
                                        : [...defaultExpenseCategories, ...defaultFundCategories])
                                    .map((c) => CategoryModel(id: c.id, name: c.name, type: c.type))
                                    .toList();
                              }
                              // If a selected id exists but isn't in the current list, schedule a background fetch
                              if (_selectedCategoryId != null && _categories.where((c) => c.id == _selectedCategoryId).isEmpty) {
                                // insert a temporary placeholder so Dropdown has a matching value immediately
                                _categories.insert(0, CategoryModel(id: _selectedCategoryId!, name: 'Selected', type: _tabIndex == 0 ? CategoryType.expense : CategoryType.fund));
                                // try to fetch a full category doc in the background and replace placeholder
                                _ensureSelectedCategoryPresent(orgId, _selectedCategoryId!);
                              }
                              // Ensure selected id refers to exactly one item; default only if null
                              if (_selectedCategoryId == null && _categories.isNotEmpty) {
                                _selectedCategoryId = _categories.first.id;
                              }
                              _selectedCategoryModel = _categories.isNotEmpty ? _categories.firstWhere((c) => c.id == _selectedCategoryId, orElse: () => _categories.first) : null;
                              return DropdownButtonFormField<String>(
                                ...
                                onChanged: (v) {
                                  setState(() {
                                    ...
                                    _selectedCategoryModel = _categories.firstWhere((c) => c.id == v, orElse: () => _selectedCategoryModel ?? _categories.first);
                                  });
                                },
                              );
                            },
                        ...
                        FutureBuilder<QuerySnapshot>(
                          future: FirebaseFirestore.instance.collection('organizations').doc(orgId).collection('categories').where('type', isEqualTo: 'fund').get(),
                          builder: (context, snap) {
                            if (snap.connectionState == ConnectionState.waiting) return const SizedBox(height:24, width:24, child: Center(child: CircularProgressIndicator(strokeWidth:2)));
                            final docs = snap.data?.docs ?? [];
                            ...
                            if (_funds.isEmpty) _funds = defaultFundCategories.map((c) => CategoryModel(id: c.id, name: c.name, type: c.type)).toList();
                            if (_selectedFundId == null || _funds.where((f) => f.id == _selectedFundId).length != 1) _selectedFundId = _funds.isNotEmpty ? _funds.first.id : null;
                            return DropdownButtonFormField<String>(
                              value: _selectedFundId,
                              decoration: InputDecoration(labelText: _tabIndex == 0 ? 'Deduct from fund' : 'Add to fund'),
                              items: _funds.map((f) => DropdownMenuItem(value: f.id, child: Text(f.name))).toList(),
                        ...
                        if (!isEdit)
                          FilledButton(
                            ...
                            style: ButtonStyle(padding: MaterialStateProperty.all(const EdgeInsets.symmetric(vertical: 14))),
                            child: _isLoading ? const SizedBox(height:18, width:18, child: CircularProgressIndicator(strokeWidth:2, color: Colors.white)) : const Text('Add Transaction'),
                          )
                        else
                          Row(
                            children: [
                              Expanded(
                                child: FilledButton(
                                  onPressed: _isLoading ? null : _save,
                                  style: ButtonStyle(padding: MaterialStateProperty.all(const EdgeInsets.symmetric(vertical: 14))),
                                  child: _isLoading ? const SizedBox(height:18, width:18, child: CircularProgressIndicator(strokeWidth:2, color: Colors.white)) : const Text('Save Changes'),
                                ),
                              ),
  ...
  Future<void> _ensureSelectedCategoryPresent(String? orgId, String selectedId) async {
    if (orgId == null) return;
    try {
      final doc = await FirebaseFirestore.instance.collection('organizations').doc(orgId).collection('categories').doc(selectedId).get();
      if (!mounted) return;
      if (doc.exists) {
